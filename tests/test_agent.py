...
def semantic_extractor_response():
    return {
        "concepts": ["Monarch butterfly", "Butterfly", "Orange", "Black"],
        "relationships": [
            {
...
def test_foo(
    ...
    mock_interaction_classifier,
    interaction_classifier_response,
    semantic_extractor_response,
):
    agent = Agent()
    ...
    differences = DeepDiff(
        message_classification, interaction_classifier_response, ignore_order=True
    )
    assert not differences, differences

    differences = DeepDiff(
        message_semantics, semantic_extractor_response, ignore_order=True
    )
    assert not differences, differences
